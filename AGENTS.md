...
| `service-prototype.html` | **전체 서비스 프로토타입**. 가입 → 온보딩(기본 정보 / 과목·자료·시험 등록 / 자료 분석 / 플래너 초안) → 화면 A·B·C. 상태는 브라우저 `localStorage`(`studymeta.service-prototype.v1`)에만 저장 | ✅ |
...
- **주 사용자**: 전공 공부 중인 대학생 + 기업 내 직무교육 학습자 (교사/연구자·개발자용 화면 아님)
- **화면 구성**: A(과목별 학습 현황) / B(최근 학습 기록) / C(학습 플래너) + 온보딩
- **리뷰 전용 UI 없음**: 화면 전환 스위처, "데모 초기화" 버튼, 프로토타입 안내 문구 같은
  개발자·리뷰어용 장치를 두지 않는다. 실제 사용자가 마주치는 화면 그대로 유지한다
  (design.md 18번)
- **학습이 일어나는 위치**: **홈이 아니라 MCP로 연결된 프론티어 모델 앱**(Claude, ChatGPT 등).
  홈은 학습 현황·기록·플래너만 보여주는 관제탑이며, 문제를 내거나 채점하지 않는다
  (design.md 16번). **홈에 문제 풀이 화면을 다시 만들지 않는다.**
- **과목**: 프리셋 선택이 아니라 학생이 직접 등록
- **학습자료 업로드**: 스킵 가능 (필수 아님)
- **시험 일정**: 과목당 여러 개 등록 가능
- **온보딩 진단 테스트**: 없음 (자료 분석 + 연결된 앱에서의 실제 학습으로 State를 채움)
...
### 3-4. `service-prototype.html`에서 지켜야 할 것

- **홈은 학습을 만들지 않는다.** 문제를 내거나 채점하는 화면(연습 화면)을 다시 추가하지
  않는다. 화면 B는 "최근 학습 기록" 열람 화면이다 — MCP로 연결된 앱에서 들어온 결과를
  보여줄 뿐, 그 자리에서 답을 받지 않는다(design.md 16번).
- **화면 B에 시나리오 선택기(개발자용 스위처)를 다시 넣지 않는다.** `viewRecord()`는
  스킬의 `lastRecord.scenarioId`로 이미 반영된 기록만 조회해서 보여준다. 반영에 쓰는
  이벤트 형태는 **9개 시나리오에서만** 가져온다(`scenarios`). 새 상황이 필요하면 시나리오를
  지어내지 말고 사용자에게 먼저 확인한다.
- **기록을 스킬에 반영할 때는 `stateUpdates`에 있는 SKILL State만 건드린다.**
  언급되지 않은 State는 그대로 둔다(모델 3원칙 중 "Evidence 없는 State는 유지").
  값을 합산하지 않고 방향(`up`/`down`/`neutral`)만 갱신한다. 반영 시 `skill.lastRecord`에
  출처(`source`)와 시점을 함께 남겨 화면 B 상단에 "어디서 온 기록인지" 보여준다.
- **자료를 다시 분석해도 기존 스킬을 갈아끼우지 않는다.** 아직 분석하지 않은 자료
  (`material.analyzed !== true`)에서 찾은 개념만 추가한다. 기존 스킬을 덮어쓰면 그동안
  쌓인 State가 사라진다.
- **플래너 카드는 (시험 × 스킬 × 배지) 단위**로 만들고 합치지 않는다. 시험일 이후 날짜
  칸은 잠그고, 드래그가 어려운 터치 환경을 위해 ◀▶ 이동 버튼을 함께 둔다.
- **데모 데이터는 과목 1개(미적분학)만 유지한다.** 여러 과목 시연이 다시 필요해지면
  사용자에게 먼저 확인 후 추가한다 — 임의로 화학/경제학원론 등을 되살리지 않는다.
- **이모지를 쓰지 않는다.** 상태·배지·결과 표시는 텍스트와 색상 강조(디자인 토큰)로만
  전달한다. 아이콘이 필요하면 이모지 대신 사용자에게 먼저 확인한다.
- **온보딩 기본 정보 필드는 중립 표현을 쓴다.** "학교/학년/전공"처럼 대학생 전용 표현
  대신 "소속/직무 또는 전공"처럼 대학생과 기업 학습자 모두에게 자연스러운 라벨을 쓴다
  (design.md 18-5).
- **MCP 연동은 예시 과목의 Chain Rule 스킬 하나로 제한한다.** `mcpDomain`/
  `mcpSkillId`를 다른 스킬이나 학생이 직접 등록한 과목에 임의로 붙이지 않는다
  — domain/skill_id 매핑 규칙이 아직 정해지지 않았다(design.md 19-3). 원점수는
  `lib/learner-mapping.js`(서버 쪽)에서만 다루고 브라우저로 보내지 않는다.

## 4. 코딩 규칙

- **화면(HTML)은 단일 파일**: `index.html`, `service-prototype.html`은 각각
  HTML + CSS + Vanilla JS를 한 파일에 담는다. 빌드 도구, 프레임워크, 외부
  CDN 의존성을 추가하지 않는다. (MCP 연동용 백엔드는 예외 — 아래 4-1 참고)
- **백엔드(`api/`, `lib/`, `scripts/`)는 최소한으로만**: MCP 서버는 브라우저가
  직접 부를 수 없어서 생긴 프록시다. Vercel 서버리스 함수 컨벤션(`api/*.js`가
  `(req, res)` 핸들러 하나를 export)만 쓰고, 프레임워크(Express 등)를
  끌어오지 않는다. 새 npm 의존성을 추가하기 전에 정말 필요한지 먼저 확인한다.
  자세한 구조와 팀원이 채워야 할 연결 정보는 `MCP_INTEGRATION.md` 참고.
