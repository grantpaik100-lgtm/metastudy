...
- **문제 풀이 결과는 9개 시나리오에서만 가져온다.** 연습 화면의 학생 행동 버튼은 각각
  기존 시나리오 id로 연결되어 있다(`PRACTICE_ACTIONS`). 새 상황이 필요하면 시나리오를
  지어내지 말고 사용자에게 먼저 확인한다.
- **피드백을 스킬에 반영할 때는 `stateUpdates`에 있는 SKILL State만 건드린다.**
  언급되지 않은 State는 그대로 둔다(모델 3원칙 중 "Evidence 없는 State는 유지").
  값을 합산하지 않고 방향(`up`/`down`/`neutral`)만 갱신한다.
